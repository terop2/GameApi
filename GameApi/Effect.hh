...
class SurfaceIn3d : public CollectInterface
{
public:
  virtual ~SurfaceIn3d() { }
...
public:
  ...
  void Collect(CollectVisitor &vis) { p1->Collect(vis); p2->Collect(vis); }
  void HeavyPrepare() { }
  int NumLines() const 
  {
    int s = p1->NumFaces();
...
public:
  ...
  void update_faces_cache() const
  {
    ...
    int k = 0;
    int s = NumFaces();
    if (s<=0||s>100000000) return; // there was limit of 200000 here, but it was removed. (the limit is needed because NumFaces() can give trash values if prepare has not been called)
...
public:
  ...
  void Collect(CollectVisitor &vis) { const_cast<BoxableFaceCollection&>(next).Collect(vis); }
  void HeavyPrepare() { }
  virtual void SetBox(Matrix b) { /*const_cast<BoxableFaceCollection&>(next).SetBox(b);*/ }
  virtual int NumFaces() const { return next.NumFaces(); }
  virtual int NumPoints(int face) const { return next.NumPoints(face); }
...
Low_SDL_Surface *InitSDL2(int scr_x, int scr_y, bool vsync, bool antialias=false, bool resize=false, bool vr_init=false);
