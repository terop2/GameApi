...
struct EnvImpl
{
  std::vector<Point, GameApiAllocator<Point> > pt;
  //std::vector<MatrixInterface*> matrix;
  GameApi::PT cursor_pos_point_id;
  std::vector<Vector, GameApiAllocator<Vector> > vectors;
  std::vector<Color, GameApiAllocator<Color> > colors;
  std::vector<VolumeObject*, GameApiAllocator<VolumeObject*> > volumes;
  std::vector<FloatVolumeObject*, GameApiAllocator<FloatVolumeObject*> > floatvolumes;
  std::vector<BitmapHandle*, GameApiAllocator<BitmapHandle*> > bm;
  std::vector<BitmapArray2<Color> *, GameApiAllocator<BitmapArray2<Color>* > > bm_array;
  std::vector<BoolBitmap, GameApiAllocator<BoolBitmap> > bool_bm;
  std::vector<FloatBitmap, GameApiAllocator<FloatBitmap> > float_bm;
  std::vector<SpaceImpl, GameApiAllocator<SpaceImpl> > sp;
  std::vector<LineImpl, GameApiAllocator<LineImpl> > ln;
  std::vector<SpritePosImpl, GameApiAllocator<SpritePosImpl> > spr_pos;
  std::vector<AnimImpl, GameApiAllocator<AnimImpl> > anim;
  std::vector<FaceCollPolyHandle*, GameApiAllocator<FaceCollPolyHandle*> > poly;
  std::vector<std::shared_ptr<void> > deletes;
  std::vector<FunctionImpl, GameApiAllocator<FunctionImpl> > func;
  std::vector<SurfaceImpl, GameApiAllocator<SurfaceImpl> > surfaces;
  //std::vector<VBOObjects*> vbos;
  std::vector<IDImpl, GameApiAllocator<IDImpl> > ids;
  std::vector<NDim<float,Point>*, GameApiAllocator<NDim<float,Point>*> > dims;
  std::vector<StateInfo2, GameApiAllocator<StateInfo2> > states;
  std::vector<StateRange, GameApiAllocator<StateRange> > state_ranges; // ST type points to this array
  std::vector<ContinuousBitmap<Color>*, GameApiAllocator<ContinuousBitmap<Color> *> > continuous_bitmaps;
  std::vector<VertexArraySet*, GameApiAllocator<VertexArraySet*> > vertex_array;
  std::vector<RenderVertexArray*, GameApiAllocator<RenderVertexArray*> > vertex_array_render;
  std::vector<int, GameApiAllocator<int> > texture_id;
  std::vector<std::string, GameApiAllocator<std::string> > shader_float_parameter;
  std::vector<std::string, GameApiAllocator<std::string> > shader_int_parameter;
  
  std::vector<Voxel<unsigned int>*, GameApiAllocator<Voxel<unsigned int>*> > voxels;
  std::vector<Voxel<int>*, GameApiAllocator<Voxel<int>*> > voxels2;
  std::vector<TROArray*, GameApiAllocator<TROArray*> > timeranges; // statechange time ranges
  std::vector<VArray*, GameApiAllocator<VArray*> > timerange_vertexarrays; // statechange vertex arrays
  std::vector<ShaderPriv2*, GameApiAllocator<ShaderPriv2*> > shaders;
  std::vector<TextureI*, GameApiAllocator<TextureI*> > textures;
  std::vector<TexCoordQuad, GameApiAllocator<TexCoordQuad> > tex_quads;
  std::vector<Separate*, GameApiAllocator<Separate*> > separates;
  std::vector<PlanePoints2d*, GameApiAllocator<PlanePoints2d*> > plane_points;
  std::vector<PlaneData, GameApiAllocator<PlaneData> > plane_array;
  std::vector<Waveform*, GameApiAllocator<Waveform*> > waveforms;
  std::vector<Array<int, ObjectWithPos> *, GameApiAllocator<Array<int,ObjectWithPos>* >  > object_move;
  std::vector<std::vector<VertexArrayWithPos> *, GameApiAllocator<std::vector<VertexArrayWithPos>*> > object_move_vertex_array;
  std::vector<Bitmap<float>*, GameApiAllocator<Bitmap<float>* > > layout_data;
  std::map<int, ArrayRender*> renders; // BM.id -> arrayrender
  std::map<int, ArrayRender*> renders2; // BM.id -> arrayrender
  std::vector<Layout*, GameApiAllocator<Layout*> > layouts;
  std::vector<PointArray2*, GameApiAllocator<PointArray2*> > pointarray;
  std::vector<PointArray3*, GameApiAllocator<PointArray3*> > pointarray3;
  std::vector<PointCollection*, GameApiAllocator<PointCollection*> > pointcollarray;
  std::vector<LineCollection*, GameApiAllocator<LineCollection*> > linearray;
  std::vector<ColorVolumeObject*, GameApiAllocator<ColorVolumeObject*> > colorvolume;
  std::map<int, ShaderPriv2*> shader_privs;
  std::vector<DistanceRenderable*, GameApiAllocator<DistanceRenderable*> > distvolume;
  std::vector<VectorVolumeObject*, GameApiAllocator<VectorVolumeObject*> > vectorvolume;
  std::vector<PointsApiPoints*, GameApiAllocator<PointsApiPoints*> > pointsapi_points;
  std::vector<LazyValue<float>*, GameApiAllocator<LazyValue<float>*> > floats;
  std::vector<Array<int,float>*, GameApiAllocator<Array<int,float>*> > float_array;
  std::vector<FBOPriv, GameApiAllocator<FBOPriv> > fbos;
  std::vector<Samples*, GameApiAllocator<Samples*> > samples;
  std::vector<Tracker*, GameApiAllocator<Tracker*> > trackers;
  std::vector<TBuffer, GameApiAllocator<TBuffer> > tracker_buf;
  std::vector<Wavs, GameApiAllocator<Wavs> > wavs;
  std::vector<ShaderModule*, GameApiAllocator<ShaderModule*> > shader_module;
  std::vector<GuiWidget*, GameApiAllocator<GuiWidget*> > widgets;
  std::vector<GameApiModule *, GameApiAllocator<GameApiModule*> > gameapi_modules;
  std::vector<FontAtlasInfo*, GameApiAllocator<FontAtlasInfo*> > font_atlas;
  std::vector<MainLoopItem*, GameApiAllocator<MainLoopItem*> > main_loop;
  std::vector<std::vector<int>*, GameApiAllocator<std::vector<int>*> > arrays;
  std::vector<ExprNode*, GameApiAllocator<ExprNode*> > exprs;
  std::vector<PhysicsNode*, GameApiAllocator<PhysicsNode*> > phys;
  std::vector<TriStrip*, GameApiAllocator<TriStrip*> > tri_strip;
  std::vector<Cutter*, GameApiAllocator<Cutter*> > cutters;
  std::vector<BO_Impl, GameApiAllocator<BO_Impl> > boolean_ops;
  std::vector<PD_Impl, GameApiAllocator<PD_Impl> > polydistfield;
  std::vector<std::vector<int>, GameApiAllocator<std::vector<int> >  > handle_array;
  std::vector<std::vector<int>, GameApiAllocator<std::vector<int> >  > template_array;
  std::vector<Collision*, GameApiAllocator<Collision*> > collision_array;
  std::vector<Movement*, GameApiAllocator<Movement*> > movement_array;
  std::vector<ImplicitFunction3d*, GameApiAllocator<ImplicitFunction3d*> > implicit;
  std::vector<TreeLevel*, GameApiAllocator<TreeLevel*> > tree_levels;
  std::vector<TreeStack*, GameApiAllocator<TreeStack*> > trees;
  std::vector<Material*, GameApiAllocator<Material*> > materials;
  std::vector<ShaderCall*, GameApiAllocator<ShaderCall*> > shadercalls;
  std::vector<Curve<Point>*, GameApiAllocator<Curve<Point>*> > curves;
  std::vector<MatrixArray*, GameApiAllocator<MatrixArray*> > matrix_arrays;
  std::vector<Curve<Matrix>*, GameApiAllocator<Curve<Matrix>*> > matrix_curves;
  std::vector<PlaneShape*, GameApiAllocator<PlaneShape*> > plane_shapes;
  std::vector<SkeletalNode*, GameApiAllocator<SkeletalNode*> > skeletals;
  std::vector<std::shared_ptr<void> > temp_deletes;
  std::vector<Pa_Impl, GameApiAllocator<Pa_Impl> > polygon_array;
  std::vector<Va_Impl, GameApiAllocator<Va_Impl> > va_array;
  std::vector<MixedI*, GameApiAllocator<MixedI*> > mixed;
  std::vector<ColorChange*, GameApiAllocator<ColorChange*> > color_change;
  std::vector<Blocker*, GameApiAllocator<Blocker*> > blockers;
  std::vector<Splitter*, GameApiAllocator<Splitter*> > splitters;
  std::vector<VertexAnimNode*, GameApiAllocator<VertexAnimNode*> > vertex_anims;
  std::vector<PointTransform*, GameApiAllocator<PointTransform*> > point_transforms;
  std::vector<CurvePos*, GameApiAllocator<CurvePos*> > curve_pos;
  std::vector<ArrayType*, GameApiAllocator<ArrayType*> > arrays2;
  std::vector<Fetcher<int>*, GameApiAllocator<Fetcher<int>*> > int_fetchers;
  std::vector<Fetcher<std::string>*, GameApiAllocator<Fetcher<std::string>*> > string_fetchers;
  std::vector<Fetcher<float>*, GameApiAllocator<Fetcher<float>*> > float_fetchers;
  std::vector<Fetcher<Point>*, GameApiAllocator<Fetcher<Point>*> > point_fetchers;
  ASyncLoader *async_loader;
  std::vector<GlyphInterface*, GameApiAllocator<GlyphInterface*> > glyph_interfaces;
  std::vector<FontInterface*, GameApiAllocator<FontInterface*> > font_interfaces;
  std::vector<StringDisplay*, GameApiAllocator<StringDisplay*> > string_displays;
  std::vector<CmdExecute*, GameApiAllocator<CmdExecute*> > cmds;
  std::vector<CurvePatch*, GameApiAllocator<CurvePatch*> > curve_patches;
  std::vector<PropertyArray*, GameApiAllocator<PropertyArray*> > prop_array;
  std::vector<Space3d*, GameApiAllocator<Space3d*> > space_3d;
  std::vector<LinesInPlane*, GameApiAllocator<LinesInPlane*> > plane_lines;
  std::vector<PointsInPlane*, GameApiAllocator<PointsInPlane*> > plane_points2;
  std::vector<FacesInPlane*, GameApiAllocator<FacesInPlane*> > plane_faces;
  std::map<int, TextureID*> txids;
  std::vector<ShaderI*, GameApiAllocator<ShaderI*> > shader_interface;
  std::vector<ContinuousBitmap<float>*, GameApiAllocator<ContinuousBitmap<float>*> > cont_floats;
  std::vector<ContinuousBitmap<bool>*, GameApiAllocator<ContinuousBitmap<bool>*> > cont_bools;
  std::vector<DynamicChange*, GameApiAllocator<DynamicChange*> > dyn_change;
  std::vector<std::vector<float> *, GameApiAllocator<std::vector<float>*> > polynomials;
  std::vector<ShaderBitmap*, GameApiAllocator<ShaderBitmap*> > shaderbitmaps;
  std::vector<DiskStore*, GameApiAllocator<DiskStore*> > disk_store;
  std::vector<Bitmap<int>*, GameApiAllocator<Bitmap<int>*> > int_bitmaps;
  std::vector<InputForMoving*, GameApiAllocator<InputForMoving*> > move_input;
  std::vector<CurveGroup*, GameApiAllocator<CurveGroup*> > curve_group;
  std::vector<MeshAnim*, GameApiAllocator<MeshAnim*> > mesh_anim;
  std::vector<FrameBufferLoop*, GameApiAllocator<FrameBufferLoop*> > frame_loop;
  std::vector<FrameBuffer*, GameApiAllocator<FrameBuffer*> > frame_buffer;
  std::vector<HeavyOperation*, GameApiAllocator<HeavyOperation*> > heavys;
  std::vector<FrmWidget*, GameApiAllocator<FrmWidget*> > frm_widgets;
  std::vector<VoxelArray*, GameApiAllocator<VoxelArray*> > voxel_array;
  std::vector<Fetcher<FaceID>*, GameApiAllocator<Fetcher<FaceID>*> > uv;
  std::vector<MatrixArray3*, GameApiAllocator<MatrixArray3*> > matrixarray3;
  std::vector<Html*, GameApiAllocator<Html*> > html;
  std::vector<DynMainLoop*, GameApiAllocator<DynMainLoop*> > dyn;
  std::vector<VelocityField*, GameApiAllocator<VelocityField*> > vfi;
  std::vector<FloatField*, GameApiAllocator<FloatField*> > ffi;
  std::vector<FloatArray2*, GameApiAllocator<FloatArray2*> > farray;
  std::vector<MemoryBlock*, GameApiAllocator<MemoryBlock*> > memblock;
  std::vector<UrlMemoryMap*, GameApiAllocator<UrlMemoryMap*> > urlmemmap;
  std::vector<Attach*, GameApiAllocator<Attach*> > attach;
  std::vector<GameState*, GameApiAllocator<GameState*> > game_state;
  std::vector<GraphicsContext*, GameApiAllocator<GraphicsContext*> > gc;
  std::vector<GLTFModelInterface*, GameApiAllocator<GLTFModelInterface*> > tf;
  std::vector<ShaderI2*, GameApiAllocator<ShaderI2*> > shi;
  std::vector<ShaderParameterI*, GameApiAllocator<ShaderParameterI*> > shp;
  std::vector<PixelBufferObject*, GameApiAllocator<PixelBufferObject*> > pbo;
  std::vector<ShaderCode*, GameApiAllocator<ShaderCode*> > shc;
  std::vector<ScreenSpaceMaterial*, GameApiAllocator<ScreenSpaceMaterial*> > ss_mat;
  std::vector<IWorld*, GameApiAllocator<IWorld*> > worlds;
  std::vector<ICache*, GameApiAllocator<ICache*> > caches;
  std::vector<Platform*, GameApiAllocator<Platform*> > platforms;
  std::vector<Timing*, GameApiAllocator<Timing*> > timings;
  std::vector<GameApi::GlobalIlluminationData*, GameApiAllocator<GameApi::GlobalIlluminationData*> > globalillumination;
  std::vector<MainLoopItemWGPU*, GameApiAllocator<MainLoopItemWGPU*> > wgpu_main_loop;
  std::vector<ColourSpace*, GameApiAllocator<ColourSpace*> > colourspaces;
  std::vector<ColourSpaceI*, GameApiAllocator<ColourSpaceI*> > colourspacesI;
  std::vector<ByteStore*, GameApiAllocator<ByteStore*> > bytestores;
  std::vector<OptVoxel*, GameApiAllocator<OptVoxel*> > opt_voxels;
  std::vector<Bitmap<FaceCollection*> *, GameApiAllocator<Bitmap<FaceCollection*> *> > p_matrix;
