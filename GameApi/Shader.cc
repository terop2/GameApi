...
void Program::set_var(const char *name, float val)
{
  ...
  if (g_setvar_cache.find(loc)==g_setvar_cache.end() || fabs(g_setvar_cache[loc]-val)>0.000001f)
    {
      g_low->ogl->glUniform1f(loc, val);
        g_setvar_cache[loc]=val;
    }
}
...
void Program::set_var(const char *name, float val1, float val2)
{
  ...
  Low_GLint loc2= 1000+loc;

  
  if (g_setvar_cache.find(loc)==g_setvar_cache.end() || fabs(g_setvar_cache[loc]-val1)>0.000001f||g_setvar_cache.find(loc2)==g_setvar_cache.end() || fabs(g_setvar_cache[loc2]-val2)>0.000001f)
    {
  g_low->ogl->glUniform2f(loc, val1, val2);
  g_setvar_cache[loc]=val1;
  g_setvar_cache[loc2]=val2;
    }
}
void Program::set_var(const char *name, const Point &p)
{
  ...
  if (g_setvar_cache.find(loc)==g_setvar_cache.end() || fabs(g_setvar_cache[loc]-p.x)>0.000001f || g_setvar_cache.find(loc2)==g_setvar_cache.end() || fabs(g_setvar_cache[loc2]-p.y)>0.000001f || g_setvar_cache.find(loc3)==g_setvar_cache.end() || fabs(g_setvar_cache[loc3]-p.z)>0.000001f)
    {
      g_low->ogl->glUniform3f(loc, p.x,p.y,p.z);
        g_setvar_cache[loc]=p.x;
        g_setvar_cache[loc2]=p.y;
      g_setvar_cache[loc3]=p.z;
    }
}
void Program::set_var(const char *name, const Vector &v)
{
  ...
  Low_GLint loc2 = 1000+loc;
  Low_GLint loc3 = 2000+loc;
  
  
  if (g_setvar_cache.find(loc)==g_setvar_cache.end() || fabs(g_setvar_cache[loc]-v.dx)>0.000001f || g_setvar_cache.find(loc2)==g_setvar_cache.end() || fabs(g_setvar_cache[loc2]-v.dy)>0.000001f || g_setvar_cache.find(loc3)==g_setvar_cache.end() || fabs(g_setvar_cache[loc3]-v.dz)>0.000001f)
    {

  g_low->ogl->glUniform3f(loc, v.dx,v.dy,v.dz);
  g_setvar_cache[loc]=v.dx;
  g_setvar_cache[loc2]=v.dy;
  g_setvar_cache[loc3]=v.dz;
    }
}
...
{
  Low_GLint loc;
  uint32_t id = unique_id(name.c_str());
  if (locs.find(id)==locs.end()) {
...
void Program::set_var(const char *name, float val1, float val2, float val3, float val4)
{
  ...
  g_setvar_cache[loc]=val1;
  g_setvar_cache[loc2]=val2;
  g_setvar_cache[loc3]=val3;
  g_setvar_cache[loc4]=val4;
...
void Program::set_var(const char *name, float *array, int count)
{
  ...
  if (g_setvar_cache.find(loc)==g_setvar_cache.end() || fabs(g_setvar_cache[loc]-array[0])>0.000001f)
    {

  g_low->ogl->glUniform1fv(loc, count, array);
  g_setvar_cache[loc]=array[0];
  }
}
...
void Program::set_var(const char *name, int val)
{
  ...
  if (g_intvar_cache.find(loc)==g_intvar_cache.end()||g_intvar_cache[loc]!=val)
     {
  
  
  g_low->ogl->glUniform1i(loc, val);  
  g_intvar_cache[loc] = val;
    }
}
...
void Program::set_var_matrix2(const char *name, const std::vector<float> &v, int start)
{
  ...
  //std::cout << ss.str() << " " << id << " " << loc << std::endl;
  ...
  for(;i!=locs.end();i++)
    {
      ...
      std::cout << "locs:" << ii << "::" << p.first << "=" << p.second << std::endl;
	ii++;
    }
}
...
void Program::set_var(const char *name, Matrix m)
{
  ...
  for(int i=0;i<16;i++)
    {
      if (fabs(m.matrix[i]-m2.matrix[i])>0.00000001f) { same=false; break; }
    }
...
"#endif\n"
    "#ifdef INSTMAT\n"
    "  vec4 p = vec4(in_Position,1.0)*in_InstMat;\n"
    "  ex_Position = vec3(p);\n" 
    "#endif\n"
"  return pos;\n"
"}\n"
...
"   ex_Normal = normalize((vec4(in_Normal,0.0)*in_N).xyz);\n"
"   return pos;\n"
"}\n"
"#endif\n"
"#endif\n"
...
"#endif\n"
    ...
    "  vec4 p = vec4(in_Position,1.0)*in_InstMat;\n"
    "  ex_Position = vec3(p);\n" 
    //"  ex_Position = vec3(vec4(in_Position,1.0) * in_InstMat);\n"
    "#endif\n"
"  return pos;\n"
"}\n"
"#endif\n"
...
"uniform vec3 in_PhongLightPos;\n"
    "vec4 phong(vec4 pos)\n"
    "{\n"
    //"    vec3 n = ize(mat3(in_View)*mat3(in_iMV)*(-in_Normal));\n"
    "    vec3 n = mat3(in_View) *mat3(in_iMV) *(-in_Normal);\n"
    "    ex_Normal2 = n;\n"
    "    ex_Position = mat3(in_View) *mat3(in_iMV) *in_Position;\n"
"#ifdef INST\n"
"  ex_Position = mat3(in_View) *mat3(in_iMV) *(in_Position + in_InstPos);\n" //mat3(in_View2)*mat3(in_iMV)*(in_Position + in_InstPos);\n"
"#endif\n"
    "#ifdef INSTMAT\n"
    "  vec4 p = vec4(in_Position,1.0)*in_InstMat;\n"
    "  ex_Position = mat3(in_View) *mat3(in_iMV) *vec3(p);\n" 

    //"  ex_Position = mat3(in_View) *mat3(in_iMV) *vec3(vec4(in_Position,1.0)*in_InstMat);\n" //mat3(in_View2)*mat3(in_iMV)* (in_Position * in_InstMat);\n"
    "#endif\n"
...
"uniform mat4 resize_matrix;\n"
"uniform mat4 resize_matrix_inv;\n"
    //"uniform mat4 inverseBind[64];\n"
"uniform mat4 jointMatrix[200];\n"
    //"uniform mat4 bindMatrix[50];\n"
    //"uniform mat4 resize[50];\n"
    //"uniform mat4 resizei[50];\n"
    "mat4 getSkinningMatrix()\n"
"{\n"
    //"float j = JOINTS_0.x;\n"
    //"if (WEIGHTS_0.x < 0.001) j=JOINTS_0.y; else\n"
    //"if (WEIGHTS_0.y < 0.001) j=JOINTS_0.z; else\n"
    //"if (WEIGHTS_0.z < 0.001) j=JOINTS_0.w;\n"
    "mat4 skin = mat4(0);\n" //resizei[int(j)]*bindMatrix[int(j)];\n"
    "skin += \n"
    ...
    "    jointMatrix[int(JOINTS_0.x)] +\n"
    "    jointMatrix[int(JOINTS_0.y)] +\n"
    "    jointMatrix[int(JOINTS_0.z)] +\n"
    "    jointMatrix[int(JOINTS_0.w)];\n"
...
"{\n"
     ...
     "vec4 v = resize_matrix * getSkinningMatrix() * resize_matrix_inv * pos;\n"
  "#ifdef INST\n"
  "  v+=in_InstPos;\n"
  "#endif\n"
  "#ifdef INSTMAT\n"
...
"float intensity(vec3 N) {\n"
  " vec3 L = normalize(ex_Position-ex_LightPos2);\n"
 ...
 " val+=0.30;\n" 
  " return 1.0-val;\n"
    "}\n"
  "}\n"
"float intensity2(vec3 N) {\n"
  //" float i = max(dot(N,normalize(ex_LightPos2)),0.0);\n"
  " float i = intensity(N);\n"
  " return pow(i,hilight);\n"
"}\n"
...
"#ifdef PHONG_TEXTURE\n"
    "    c+=SRGBtoLINEAR2(level1_color.rgb+rgb.rgb);\n"
  "    c=mix(c,SRGBtoLINEAR2(level2_color.rgb+rgb.rgb),intensity(normal));\n" //mix(SRGBtoLINEAR2(level2_color.rgb)*((SRGBtoLINEAR2(rgb.rgb)+SRGBtoLINEAR2(level1_color.rgb))/2.0),SRGBtoLINEAR2(vec3(1,1,1)),0.15);\n"
  //"    c=mix(c,SRGBtoLINEAR2(level3_color.rgb),intensity2(normal));\n"
   " c=mix(c,SRGBtoLINEAR2(level3_color.rgb+rgb.rgb),intensity2(normal));\n"
"#endif\n"
"#ifndef PHONG_TEXTURE\n"
    "    c+=SRGBtoLINEAR2(level1_color.rgb);\n"
    "    c=mix(c,SRGBtoLINEAR2(level2_color.rgb),intensity(normal));\n"
  "    c=mix(c,SRGBtoLINEAR2(level3_color.rgb),intensity2(normal));\n"
"#endif\n"
...
"#ifdef BUMPPHONG\n"
    ...
    //"}\n"
"float intensitya(vec3 N) {\n"
  " vec3 L = normalize(ex_LightPos2);\n"
  " vec3 V = normalize(vec3(0.0,0.0,-400.0)-ex_Position);\n"
  " vec3 H2 = normalize(L+V);\n"
  " float n = max(dot(-N,H2),0.2);\n"
" return n;\n"
"}\n"
"float intensity2a(vec3 N) {\n"
  //" float i = max(dot(N,normalize(ex_LightPos2)),0.0);\n"
  " float i = intensitya(N);\n"
  " return pow(i,hilight);\n"
"}\n"
...
"{\n"
" return vec4(pow(c.xyz, vec3(INV_GAMMA_txt)),c.w);\n"
"}\n"
...
"uniform float color_mult_self;\n"
"uniform float color_mult_rest;\n"
...
"#endif\n"
  "#ifdef INSTMAT\n"
    "  vec4 p = vec4(in_Position,1.0)*in_InstMat;\n"
    "  ex_Position = vec3(p.x/p.w,p.y/p.w,p.z/p.w);\n" 
  //"  ex_Position = vec3(vec4(in_Position,1.0)*in_InstMat);\n" //mat3(in_View2)*mat3(in_iMV)* (in_Position * in_InstMat);\n"
  "#endif\n"
"  return pos;\n"
"}\n"
...
"float V_intensity(vec3 N) {\n"
  " vec3 V = normalize(ex_LightPos2);\n"
  " vec3 L = normalize(vec3(0.0,0.0,-400.0)-ex_Position);\n"
  " vec3 H2 = normalize(L+V);\n"
  " float n = max(dot(-N,H2),0.2);\n"
" return n;\n"
"}\n"
"float V_intensity2(vec3 N) {\n"
  //    " float i = max(dot(normalize(dir),normalize(ex_LightPos2)),0.0);\n"
  " float i = V_intensity(N);\n"
  " return pow(i,hilight);\n"
"}\n"
...
"}\n"
"float intensity2(vec3 N) {\n"
  //" float i = clamp(dot(normalize(dir),normalize(ex_LightPos2)),0.2,1.0);\n"
  "  float i = intensity(N);\n"
  " return pow(i,hilight);\n"
"}\n"
"vec3 normalmixp(vec3 a, vec3 b) {\n"
...
"#ifdef PHONG_TEXTURE\n"
  "    c=SRGBtoLINEAR2(level1_color.rgb+rgb.rgb);\n"
  "    c=mix(c,SRGBtoLINEAR2(level2_color.rgb+rgb.rgb),intensity(normal));\n" //mix(SRGBtoLINEAR2(level2_color.rgb)*((SRGBtoLINEAR2(rgb.rgb)+SRGBtoLINEAR2(level1_color.rgb))/2.0),vec3(1,1,1),0.15);\n"
    "    c=mix(c,SRGBtoLINEAR2(level3_color.rgb+rgb.rgb),intensity2(normal));\n"
"#endif\n"
"#ifndef PHONG_TEXTURE\n"
  "    c+=SRGBtoLINEAR2(level1_color.rgb);\n"
    "    c=mix(c,SRGBtoLINEAR2(level2_color.rgb),intensity(normal));\n"
    "    c=mix(c,SRGBtoLINEAR2(level3_color.rgb),intensity2(normal));\n"
  //" c+=intensity2(normal)*SRGBtoLINEAR2(level3_color.rgb);\n"
  "#endif\n"
...
// " float i=intensity(dir); return pow(i,hilight);\n"
// "}\n"
"float intensityb(vec3 N) {\n"
  " vec3 L = normalize(ex_LightPos2);\n"
  " vec3 V = normalize(vec3(0.0,0.0,-400.0)-ex_Position);\n"
  " vec3 H2 = normalize(V+L);\n"
  " return max(dot(N,H2),0.0);\n"
"}\n"
"float intensity2b(vec3 N) {\n"
  //" float i = clamp(dot(normalize(dir),normalize(ex_LightPos2)),0.2,1.0);\n"
  "  float i = intensityb(N);\n"
  " return pow(i,hilight);\n"
"}\n"
