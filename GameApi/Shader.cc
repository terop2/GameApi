...
#ifdef WEBGL2
    "    float d = texture(texsampler[texindex] /*in_txid*/,tx).r*800.0;\n"
#else
    "    float d = texture2D(texsampler[texindex] /*in_txid*/,tx).r*800.0;\n"
#endif
...
"    vec2 tx = vec2(tx_x,tx_y);\n"
"    float dist = length(pos - (in_pos+tx_x*in_dx+tx_y*in_dy));\n"
    //#ifdef WEBGL2
    //"    texture(in_txid,tx).r = min(texture(in_txid,tx).r,dist);\n"
    //#else
    //"    texture2D(in_txid,tx).r = min(texture2D(in_txid,tx).r,dist);\n"
...
"#endif\n"
"#ifdef EX_POSITION\n"
"in float in_NewShadowDarkLevel;\n"
"in float in_NewShadowLightLevel;\n"
"float find_projection_length(vec3 u, vec3 u_x) {\n"
"   float len = dot(u,u_x)/length(u);\n"
"   return len;\n"
"}\n"
...
"    else { level=in_NewShadowLightLevel; }\n"
"    return level*rgb;\n"
"}\n"
"#endif\n"
