...
EXPORT GameApi::BM GameApi::BitmapApi::scale_bitmap_fullscreen(EveryApi &ev, BM orig)
{
  ...
  CBM cbm = ev.cont_bitmap_api.from_bitmap(orig, 1.0, 1.0);
  BM ret = ev.cont_bitmap_api.to_bitmap(cbm, 800,600);
  return ret;
}
...
public:
  ...
  std::vector<unsigned char> *data = load_from_url(url, false);
...
static inline float lerp3(float t, float a, float b) {
    return a + t * (b - a);
}

// Gradient directions (hashed)
static inline float grad3(int hash, float x, float y, float z) {
    int h = hash & 15;             // Take low 4 bits
    float u = h < 8 ? x : y;       // Select x or y
    float v = h < 4 ? y : (h == 12 || h == 14 ? x : z);
    return ((h & 1) ? -u : u) + ((h & 2) ? -v : v);
}

// Permutation table (classic Ken Perlin)
static const uint8_t perm[512] = {
    151,160,137,91,90,15, // First 256 values...
...
float perlin(float x, float y, float z) {
    ...
    return lerp3(w,
        lerp3(v,
            lerp3(u, grad3(perm[AA],   x,   y,   z),
                    grad3(perm[BA],   x-1, y,   z)),
            lerp3(u, grad3(perm[AB],   x,   y-1, z),
                    grad3(perm[BB],   x-1, y-1, z))),
        lerp3(v,
            lerp3(u, grad3(perm[AA+1], x,   y,   z-1),
                    grad3(perm[BA+1], x-1, y,   z-1)),
            lerp3(u, grad3(perm[AB+1], x,   y-1, z-1),
                    grad3(perm[BB+1], x-1, y-1, z-1)))
    );
}

class PerlinNoise2 : public Bitmap<float>
{
public:
  PerlinNoise2(int sx, int sy, float key) : sx(sx), sy(sy),key(key) { }
  void Collect(CollectVisitor &vis) { }
  void HeavyPrepare() { }
  void Prepare() { }
  int SizeX() const { return sx; }
  ...
  float Map(int x, int y) const {
    float p = perlin(float(x)/float(sx),float(y)/float(sy),key); 
    return (p+1.0f)*0.5f;
  }
private:
  int sx,sy;
...
EXPORT GameApi::FB GameApi::FloatBitmapApi::perlin_noise2(int sx, int sy, float key)
{
